...
%FSM GENERALE:
	...
	[->,align=center,node distance=2cm, text width=10mm, ]
		...
		\node[state, below of=F3S1] (F3S2) {$MemLoad$};
		\node[state, below of=F3S2] (F3S3) {$Get\_Val$};
		\node[state, below of=F3S3] (F3S4) {$Zeroing$};
		\node[state, below of=F3S4] (F3S5) {$Shift$};
		\node[state, below of=F3S5] (F3S6) {$Overflow$};
		\node[state, below of=F3S6] (F3S7) {$MemWrite$};
		\node[state, below of=F3S7] (F3S8) {$Pixel\_Inc$};
