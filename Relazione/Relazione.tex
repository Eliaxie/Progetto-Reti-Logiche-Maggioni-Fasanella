...
\begin{tikzpicture}
    ...
    \node[state,initial,initial where=left, initial text=i\_rst] (S0) {$S0$};
...
\end{center}
 ...
 Questi stati si occupano della lettura di \emph{OP1} e \emph{OP2} (numero righe e colonne) attraverso il processo \emph{MULT}. In \emph{F1S2b} un controllo si accerterà che \emph{OP1} non sia nullo, altrimenti si passerà allo stato di terminazione e reset dei valori per una nuova lettura.
  \item F1S3,F1S3b \((Multiply)\)
  
 A questo punto viene calcolato \emph{M} (cioè \(OP1*OP2\)) iterando in questi due stati attraverso il segnale \emph{o\_m}. Il calcolo avviene per somme successive fino a quando \emph{OP2} sarà zero. 
  \item F1S4,F1S4a,F1S5 \((Pixels)\)
  
  Attraverso i processi \emph{ADDRHandler \emph{e} MINeMAX}, verrà incrementato \emph{REGAddr} (registro dell'indirizzo di memoria) e controllato Pixel (registro che salva ogni current pixel). Il segnale f1 detta il passaggio fra questi stati attraverso la condizione \(REGAddr > M + 2 \). Il calcolo del delta invece, avviene ad ogni nuovo assegnamento del Massimo o del Minimo.
  
  \item F1S6,F1S7 \((Finalize)\)
  
  Una volta terminato il proprio compito, nello stato \emph{F1S6} verrà alzato il segnale start2 per far partire il secondo modulo e successivamente, lo stato \emph{F1S7} si metterà in attesa del segnale \emph{done2} per terminare l'elaborazione e reset dei valori.
  \item F1S8,F1S9 \((Reset)\)
  
  A questo punto, una volta ricevuto il segnale \emph{done2}, lo stato \emph{F1S8} alzerà done ed \emph{endof} (segnale generale di imminente terminazione e reset dei valori), e aspetterà che si abbassi start, per poi passare a \emph{F1S9} che farà riscendere i due precedenti segnali. Si torna quindi a \emph{F1S0} in attesa di una nuova immagine.
\end{itemize}
  
    \item \textbf{Modulo 2}
		
		Il modulo due svolge il calcolo dello shift level noto il data value e lo rende disponibile per l'utilizzo al modulo tre. Una volta completato il calcolo, alza il segnale \emph{start3} per fare partire la FSM3, dopodichè si mette in attesa di \emph{done3} per poi terminare alzando per un ciclo di clock il segnale \emph{done2}. Di seguito la descrizione della FSM2:
		
		\begin{tikzpicture}
[->,align=center,node distance=1.2cm]
\node[state, initial, initial text=i\_rst] (s0) {$S0$};
...
\end{tikzpicture}
			\begin{itemize}
				...
				Carica il valore di delta nel registro \emph{S2R1}.
				
				\item F2S2,F2S3,F2S4 \emph{(LUT)}
				
				Attende la propagazione del segnale nel datapath e carica il valore in uscita dalla \emph{LUT} in \emph{S2R2}.
				
				\item F2S5 \emph{(Start3)}
				
				Avvia il modulo tre alzando il segnale \emph{start3}.
				
				\item F2S6 \emph{(WaitDone3)}
				
				Attende in questo stato finché \emph{done3} non viene alzato dal modulo tre
...
Di seguito i risultati della sintesi, ottenuti attraverso la Simulazione in ambiente Vivado. 
