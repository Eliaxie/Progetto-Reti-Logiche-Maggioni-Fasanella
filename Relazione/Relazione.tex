...
Risultati dei test fatti e le ragioni di tali test. 

\begin{LUT}
Bisogna considerare che \(\log (\delta +1)) risulta caso limite nel momento in cui il valore di \delta assume valore 2^(\n)-1 (con 0<=\n<=8). La LUT dell'implementazione, tiene già conto dell'argomento del logaritmo, cambiando quindi il valore ogni volta che viene raggiunto dal delta una potenza di 2.  
\end{LUT}

\begin{Delta=0}
Il \delta sarà nullo nel momento in cui tutti i valori dell'immagine saranno uguali. Sarà infatti \MAXPixel=\MINPixel, generando così uno \textit(shift_level)=8 (caso massimo) che porterà quindi ad avere in output solo valori nulli. Questo è anche intuitivamente immaginabile con una monocromia dell'immagine che quindi viene riportata al valore di default.
\end{Delta=0}

\begin{Delta=255}
Il \delta sarà massimo nel momento in cui \MAXPixel=255 e \MINPixel=0. Il valore del logaritmo è quindi massimo e lo \textit(shift_level)=0 (caso minimo) che porterà quindi ad avere in output gli stessi valori di input.
\end{Delta=255}

\begin{Shift Value}
Lo Shift sinistro di \n posizioni equivale a moltiplicare \value*2^(\n). Un caso importante da considerare è quando il valore di \n=8 : il valore di \textit(current_pixel-MINPixel) verrà quindi completamente portato a sinistra lasciando tutti zeri, rispetto alla rappresentazione di 8 bit del \pixel. Perciò il valore di \textit(temp_pixel)>255 e si avrà quindi un output di valori massimi.
\end{Shift Value}
...
\item TB tutti 255 \input=[2,3,0,0,0,0,0,0] \output=[0,0,0,0,0,0]

\item TB tutti 0
\item TB singolo pixel	 
\item TB: Nel Test generico si cerca di toccare tutti i principali casi limite che possono essere incontrati. Si tratta infatti di situazioni in cui i valori del Delta e, di conseguenza, 
